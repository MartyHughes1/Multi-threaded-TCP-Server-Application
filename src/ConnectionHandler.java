...
public class ConnectionHandler extends Thread {
		...
		try {
			while (true) {
				...
				if (command.equalsIgnoreCase("REGISTER")) {
					...
					handleLogin(parts);
				} else if (command.equalsIgnoreCase("CREATE_RECORD")) {
	...
	private void handleCreateRecord(String[] parts) {
		...
		if (parts.length != 3) {
			sendMessage("ERROR: Invalid CREATE_RECORD format");
			return;
		}
		...
		// Default status: Available (for book), Requested (for borrow)
		String status;
		if (recordType.equalsIgnoreCase("New Book Entry")) {
			status = "Available";
		} else {
			status = "Requested";
		}

		// Assigned librarian ID is empty at creation
		String librarian = "";

		LibraryRecord newRecord = new LibraryRecord(recordType, recordId, date, studentId, status, librarian);

		synchronized (Provider.records) {
			Provider.records.add(newRecord);
		}

		sendMessage("SUCCESS: Record created with ID: " + recordId);
	}
